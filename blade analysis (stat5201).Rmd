...
```{r, echo=FALSE}
setwd("C:/Users/jctho/OneDrive/Documents/9 UIowa classes/stat5201 Applied Stat II/project (stat5201)")
...
blade$p_group <- as.factor(blade$p_group)
#
aov0 <- aov(pass ~ p_group*trt + brand*trt, data = blade)
plot(aov0)
summary(aov0)
boxcox(aov0)
#
blade$t_pass <- (blade$pass)^(-.5)
aov01 <- aov(t_pass ~ p_group*trt + brand*trt, data = blade)
plot(aov01)
```


```{r}
EMSanova(t_pass ~ p_group + trt + brand,
         data = blade,
         type = c("R","F","R"),
         nested = c(NA, NA, "p_group"))
str(blade)
```
